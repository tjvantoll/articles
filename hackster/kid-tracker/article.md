...
My custom tracker takes GPS readings every five minutes and displays them on a map. Additionally, the tracker includes a button that kids can press their location to a parent over SMS.
...
Let’s start by looking at the hardware I used, most of which I got in one purchase of a [Blues Wireless Feather Starter Kit for Swan](https://shop.blues.io/products/notecarrier-swan). The Feather Starter kit contains a Notecard, a Notecarrier AF, and a Swan MCU. Let’s break down what each of those are.

### Notecard

The [Notecard](https://blues.io/products/notecard/) is a small system-on-module that can add connectivity to just about any IoT project. Blues makes both cellular and and Wi-Fi Notecards, but for GPS tracking you’ll basically always want to use a cellular Notecard so the tracking works outside of the range of Wi-Fi networks.
...
Notecarriers are companion boards that make it easy to prototype and deploy IoT projects with the Notecard. Essentially, the Notecarrier gives you a convenient place to connect to a Notecard to supply it with power, as well as providing bonus features like embedded antennas, and Qwiic ports for connecting I2C peripherals.

![](notecarrier-af.jpg)

The Notecarrier AF also contains Feather-compatible headers, making it easy to embed any Feather-compatible device for using alongside the Notecard.

### Swan

And the embedded Feather-compatible device I used was the [Swan](https://blues.io/products/swan/). The Swan works for most just about any IoT project, as it has a hefty 640KB of RAM, is expandable to 55 pins for access to additional I/O and buses, and has built-in support for C/C++, Arduino, and CircuitPython.

![](swan.jpg)

For my purposes the Swan is going to run the code I need to drive the entire project.

### Putting it all together

Assembling the pieces of this build mostly involves slotting both the Notecard and the Swan into the Notecarrier AF. Blues has a [good guide with pictures](https://dev.blues.io/quickstart/notecard-quickstart/notecarrier-af/) that I’d recommend following if you’re setting this up for the first time.
...
Next, I added in a small Lipo battery, and connected it to the `BATTERY` JST connector on the Notecarrier AF. The Lipo battery powers the device when I have it out and in use.
...
If you recall from step #1, I’m using a Swan MCU to execute the code that drives the kid tracker. The Swan supports both CircuitPython and Arduino, so you can use either, but I went with Arduino because it’s a platform I’m used on the Swan several times before.
...
Once you have your development environment ready you’re ready to move on to the code itself. And in the code there are three primary things we need to handle:
...
Sending data every five minutes is already fairly power hungry, but for this project I want to see data often while my kids are out on their bikes. I also have no issue charging the device’s lipo battery in between uses. If your project needs to be more conservative with battery usage, you’ll want to set the `"outbound"` value to a considerably higher number.
...
> **NOTE**: If you’re not seeing logs, you can add `while (!serialDebug)` after your `serialDebug.begin()`, which forces the program to wait until the debugger is available before continuing. Just make sure to remember to remove `while (!serialDebug)` when you’re ready to deploy your device, as it’ll cause an infinite loop if running when no serial debugger exists.

And if you return to your Notehub project you should now see your device listed.

![](device-in-notehub.png)

At this point you have your Notecard connected to Notehub and ready to send data. Let’s next look at the requests you need to send to configure the Notecard as an asset tracker.

### Configuring how often to take GPS readings

The Notecard has a number of request that allow you to configure exactly how GPS readings should be taken. There are two primary commands you need to send to the Notecard: `card.location.mode` and `card.location.track`.
...
The `card.location.track` request starts the actual tracking process. When in periodic GPS mode, the Notecard is smart enough to only take GPS readings if the device has moved (which it knows using its built-in accelerometer). Setting the `heartbeat` option to `true` on this request tells the Notecard to take a reading every 12 hours (because we set `"hours"` to `12`), even if the Notecard hasn’t moved.
...
It may take several minutes to see events as it can take some time for the Notecard to establish a GPS connection. Try to place your device in a location that has a clear line of sight to the sky, and avoid confined places like basements if possible.
...
In your code, you can start by adding the following `#define` lines to the top of your sketch.
...
There’s a lot here, so let’s break this code down. First, note that all the logic here is gated by a check an `if (!locationRequested) { return; }` check. Aka, if the user hasn’t pressed the button, there’s nothing to do in `loop()`.
...
And with that, you’re done writing the code you need to make this project run! Before pushing the code out to the device one last time, go into Arduino IDE’s **Tools** menu, and switch the **C Runtime Library** to one that support floating point printing and scanning, as we’re using that in our logic that places the latitude and longitude into a Google Maps URL.

![](arduino-options.png)

> **NOTE**:
> * Floating point support uses a _lot_ of memory so you should use some caution before putting it in any IoT project. This is one reason I chose the Swan for this project, as it comes with a hefty 640KB of RAM which is great for situations like this.
...
Perhaps the best feature of Notehub is how easy it is to send data to other places. Those places could be one of the big clouds, like AWS, Azure, or Google Cloud, they could be an IoT platform like Datacake or Ubidots, or they could be a custom HTTP API that you set up.

For my kid tracker I wanted an easy place to quickly visualize location data, so I ended up going with Datacake, as they have a Notecard template that makes setting up a dashboard as easy as clicking a few buttons.
...
Datacake also allows you to set up a mobile dashboard view, which I found help as I often want to track my kids on my phone. For the mobile view I placed the map in top, as that’s the main thing I’m interested in, and clicked the “Public Link” button to generate a URL I can load on my device.
...
As a last step before you’re ready to take your device out in the wild, let’s look at how to make the SMS messages work.
...
One of the services you can route from Notehub into is Twilio, and making it happen is as easy as signing up for a Twilio trial account, and creating a new Notehub route.
...
> **NOTE**: If you’re following this guide exactly, use `"&Body=" & body.message & "&From=+1234567890&To=+1234567890&"` (swapping in your own numbers) as your route’s **JSONata Expression**. I like to store my phone numbers on the route in Notehub, as it’s more configurable there than as a variable in my code.
...
GPS trackers are hard to test because they rely on you moving around the world. Kid trackers are even harder to test because, if you want to test them right, you have give your kids a device, send them off, and see what happens.

So I did!

Since I was most interested in bike tracking, I decided to test out the project by giving my kids the tracker, sending them off on their bikes, and telling them to press the button after a bit so I could come find them. What could go wrong?

![](kids.jpg)

As it turns out, things can go wrong 😅 My big problem is that for test #1 I had still not taped up everything in my device. Aka, the blue bag my son Chase (on the right above) is holding contains this.

![](setup-loose.jpg)

As it turns out, this is not only problematic because of potential electrical shorts (bouncing loose electrical components == a problem waiting to happen), but also dangerous, as [a punctured lipo battery is basically a small bomb](https://www.youtube.com/watch?v=wUFxlf4fXjo). Fun!

Luckily things didn’t go _that_ wrong, but about 13 minutes into my kids’ bike ride I stopped getting `_track.qo` events, and noticed this concerning “brown-out & hard reset” health event come through.
...
Second, I purchased a [small little under seat storage bag for bikes](https://www.amazon.com/gp/product/B08PNSZNP6/ref=ppx_yo_dt_b_asin_title_o00_s00?ie=UTF8&psc=1). This pouch holds the tracker more snug so it doesn’t bounce around as much, and I figure the pouch will be useful for holding keys and such in the future.
