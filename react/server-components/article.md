...
In this article I want to help you decide whether [React Server Components](https://reactjs.org/blog/2020/12/21/data-fetching-with-react-server-components.html) is something that you should check out immediately, or whether you should wait and see.
...
By using those two libraries you can write a simple component that looks something like this:
...
This is pretty cool, but before you get too excited, there are some Server Components limitations that will keep you from removing a lot of your client-side dependencies. Here’s the full list from the [React team’s Server Components RFC (Request for Comments)](https://github.com/josephsavona/rfcs/blob/server-components/text/0000-server-components.md#sharing-code-between-server-and-client).

![](limitations.png)
*List of Server Components limitations*

The big ones here are Server Components cannot have state and cannot work with DOM APIs. And most React UI components... tend to have state and work with DOM APIs. Therefore, most of your dependencies that involve the UI will still have to live on the client.
...
Server Components have the ability to help here, as you now have the ability to move data-access components to a server, which can access your data storage much faster.
...
The problem is problematic as your code must wait for three separate network requests, which is not only potentially slow, but also might lead to race conditions, depending on how you use this data.
...
React Server Components offers an interesting new approach to this problem, by allowing you to access your data directly. For example, consider this update to the header that lets you access a database right in your component.
...
And that’s just when creating the components. As Server Components advance, some of those same concerns are going to apply to how you unit test your components that can now span multiple environments, and also how you debug them as well.
...
To be fair, most of these limitations come from Server Components being so new. The React team has stated that they expect most of the initial adoption to be through frameworks like [Next.js](https://nextjs.org/) early on, so it would make sense that some of these workflows are a bit rough today. With that in mind though, should you care about Server Components.
...
These frameworks also have the ability to help hide some of the messier implementation deals of the underlying React implementation, making it easier for your average developer to use.

**2) If your company is operating at Facebook’s scale.**

The React Server Components of today introduces a lot of complexity for small performance gains.
