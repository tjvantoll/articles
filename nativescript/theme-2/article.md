...
With the [NativeScript 6.0 release](https://www.nativescript.org/blog/announcing-nativescript-6.0---webpack-all-the-way-seamless-app-updates-new-core-theme-and-a-lot-more) we announced a new beta of the NativeScript 2.0 theme, a comprehensive overhaul to the theme all NativeScript apps use by default.
...
The new NativeScript theme is already on npm, and you can change any existing app to use the new theme by running the `tns plugin update` command.
...
Next, add the following line of code to your JavaScript or TypeScript code. This will be your `app.js` or `app.ts` file for NativeScript Core and NativeScript-Vue apps, and your `main.ts` file for NativeScript Angular apps.

``` TypeScript
import "nativescript-theme-core";
```

This line of code adds the following class names to the root element of your app, which the theme uses for styling purposes. You might also find these class names useful for your own custom styling.
...
Finally, you’ll need to update the syntax to include the theme’s CSS in your `app.css` file. Find your current import which will look something like this.
...
With these steps complete you should now have the new theme up and running, so let’s next look at the updates and new features you can try out.

## Plug and play

There’s one big change to the way the new core theme works that we have to start with, and I think it’s easiest to understand by looking at a bit of code first.
...
This is no longer necessary, components get a base set of styles without any class names at all. For example here’s what the default button looks like in a NativeScript app using the new theme and the blue color scheme.
...
One super important note before we go further: **the NativeScript 2.0 theme provides full backwards compatibility with the 1.0 theme class names**. Therefore, you can update to the new theme without having to all the class names throughout your apps.

That being said, you will likely need to make some CSS changes in your app when updating. For example, suppose you use this button in your app.

``` XML
<Button text="Confirm your choice"></Button>
```

Because this button does not use the `btn` class name, it was not styled with the NativeScript 1.0 theme, but it _will_ be styled as soon as you update to the NativeScript 2.0 theme; therefore, you might need to write some custom CSS to ensure these type of components look correct in your updated app.
...
Enabling this dark mode is as easy as adding a `ns-dark` class name to the root element of your NativeScript app. There are a few different elements that might be your root element depending on how you built your app. For NativeScript-Angular apps your root element is usually your `<page-router-outlet>`, which usually lives in your `app.component.html` file.
...
And in case you’re wondering, there will soon be a JavaScript / TypeScript API for switching your app from dark mode to light mode, in case you want to provide dark mode as an option for your users to toggle. You could even [detect whether the user has dark mode enabled on their iOS device](https://github.com/EddyVerbruggen/nativescript-dark-mode), and conditionally apply a dark mode in your app based on the user’s global iOS preference.
...
[Kendo UI ThemeBuilder](https://themebuilder.telerik.com) is a tool for visually creating themes.

![](themebuilder.png)

Historically the tool was used only for web apps, but now you can use the same to configure color schemes for your NativeScript apps as well.

Once you’ve configured your theme hit the **DOWNLOAD** button in the top-right corner, which will give you a `.zip` bundle with two files, an `app.css` file you can use on the web if you’d like, and a `variables.scss` file you’ll need for using the color scheme with NativeScript.

![](file-tree.png)

One important note before we continue: to use ThemeBuilder-built color schemes in NativeScript you must use SASS in your NativeScript apps, as ThemeBuilder outputs SASS variables, which the new NativeScript theme is able to use.
...
Feel free to experiment with Kendo ThemeBuilder to create the perfect theme for your own apps.
...
The new theme is in beta, and as such we’d love to have your feedback. What do think about the updates to the theme class names? Is dark mode working like you expect it to? Do you like using Kendo UI ThemeBuilder?
